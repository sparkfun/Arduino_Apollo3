...
/*! UUIDs */
static const uint8_t wdxcDcUuid[ATT_128_UUID_LEN] = {WDXS_DC_UUID};     /* WDX Device Configuration Characteristic */
static const uint8_t wdxcFtcUuid[ATT_128_UUID_LEN] = {WDXS_FTC_UUID};   /* WDX File Transfer Control Characteristic */
static const uint8_t wdxcFtdUuid[ATT_128_UUID_LEN] = {WDXS_FTD_UUID};   /* WDX File Transfer Data Characteristic */
static const uint8_t wdxcAuUuid[ATT_128_UUID_LEN] = {WDXS_AU_UUID};     /* WDX Authentication Characteristic */
...
const uint8_t wdxcSvcUuid[ATT_16_UUID_LEN] =    {UINT16_TO_BYTES(WDXS_SVC_UUID)};
 ...
 *          WDXS_FTC_OP_EOF and the file handle equal to WDXS_FLIST_HANDLE.
 *
 *  \return None.
 */
/*************************************************************************************************/
void WdxcDiscoverFiles(dmConnId_t connId, wsfEfsFileInfo_t *pFileInfo, uint8_t maxFiles)
{
  ...
  {
    uint16_t len = maxFiles * WDXS_FLIST_RECORD_SIZE + WDXS_FLIST_HDR_SIZE;

    /* Update control information */
    pConnCb->pFileList = pFileInfo;
    pConnCb->fileCount = 0;
    pConnCb->maxFiles = maxFiles;
    pConnCb->fileHdl = WDXS_FLIST_HANDLE;
    pConnCb->fDlPos = 0;
    ...
    WdxcFtcSendGetReq(connId, WDXS_FLIST_HANDLE, 0, len, 0);
  }
}

/*************************************************************************************************/
/*!
 *  \fn     wdxsParseFileList
 *
...
static void wdxsParseFileList(dmConnId_t connId, uint8_t *pValue, uint16_t len)
{
  ...
  while (pos < len)
  {
    if (pConnCb->fDlPos < WDXS_FLIST_HDR_SIZE)
    {
    ...
    else
    {
      ...
      uint8_t mark = (pConnCb->fDlPos - WDXS_FLIST_HDR_SIZE) % WDXS_FLIST_RECORD_SIZE;
      uint8_t file = (pConnCb->fDlPos - WDXS_FLIST_HDR_SIZE) / WDXS_FLIST_RECORD_SIZE;
...
static void wdxcParseFtd(dmConnId_t connId, uint8_t *pValue, uint16_t len)
{
  ...
  if (pConnCb->fileHdl == WDXS_FLIST_HANDLE)
  {
...
/*************************************************************************************************/
/*!
 *  \fn     wdxcWdxsValueUpdate
 *        
 *  \brief  Process a value received in an ATT read response, notification, or indication 
 *          message.  Parameter pHdlList must point to an array of length WDXC_WDXS_HDL_LIST_LEN. 
