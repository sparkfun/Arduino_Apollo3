...
void Apollo3RTC::setTime(uint8_t hund, uint8_t sec, uint8_t min, uint8_t hour, uint8_t dayOfMonth, uint8_t month, uint16_t year) {
...
void Apollo3RTC::setToCompilerTime() {
...
void Apollo3RTC::setEpoch(uint32_t ts) {
    if (ts < EPOCH_TIME) {
        ts = EPOCH_TIME;
    }
...
void Apollo3RTC::setAlarm(uint8_t hund, uint8_t sec, uint8_t min, uint8_t hour, uint8_t dayOfMonth, uint8_t month) {
...
*/
void Apollo3RTC::setAlarmMode(uint8_t mode) {
    am_hal_rtc_alarm_interval_set(mode);
}

void Apollo3RTC::attachInterrupt() {
...
void Apollo3RTC::detachInterrupt() {
...
int Apollo3RTC::mthToIndex(char const *pcMon) {
  int idx;
...
int Apollo3RTC::toVal(char const *pcAsciiStr) {
  int iRetVal = 0;
