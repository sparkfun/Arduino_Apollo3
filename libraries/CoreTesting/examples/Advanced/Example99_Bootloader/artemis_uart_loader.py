...
def main():
    ...
    try:
        ...
        for dev in devices:
            ...
            if 'CH340' in dev.description:
                if(dev.device == args.port):
                    print(
                        "The port you selected is currently open. Please close any other terminal programs that may be using " + dev.device + " and try again.")
                else:
                    print("The port you selected was not found. But we did detect a CH340 on " +
                          dev.device + " so you might try again on that port.")
                break
            elif 'FTDI' in dev.description:
                print("The port you selected was not found. But we did detect an FTDI on " +
                      dev.device + " so you might try again on that port.")
                break
            elif 'USB Serial Device' in dev.description:
                print("The port you selected was not found. But we did detect a USB Serial Device on " +
                      dev.device + " so you might try again on that port.")
                break
        else:
    ...
    with serial.Serial(args.port, 9600, timeout=0.100) as ser:
        ...
        # If we do not set DTR high again, the built-in bootloader will not activate, but board should reset

        print("Waiting for command from bootloader")
        ...
        if(ord(response) == BL_COMMAND_ANNOUNCE):
            ...
            print("Bootload response received")
        else:
        ...
        print("Length to send: " + str(totalLen))
        ...
        while 1:
            ...
            # print('myCRC32: {:#010x}'.format(myCRC32))
            ...
            if ord(response) == BL_COMMAND_NEXT_FRAME:
                print("Sending next frame: " + str(end - start) + " bytes")
                ...
                # Break the write into stages to give time inbetween
                chunks = 1
                i = 0
                while i < chunks:
                    partialStart = int(start + (i * (bytes_to_send/chunks)))
                    partialEnd = int(
                        end - ((chunks - 1 - i) * (bytes_to_send/chunks)))

                    ser.write(application[partialStart:partialEnd])
...
def connect_device(ser):
    ...
    if ((word & 0xFFFF) == AM_SECBOOT_WIRED_MSGTYPE_STATUS):
        ...
        if (args.binfile != ''):
            ...
            numUpdates = (totalLen + maxUpdateSize -
                          1) // maxUpdateSize  # Integer division
            ...
            for numUpdates in range(numUpdates, 0, -1):
                ...
                applen = end - start
                verboseprint("Sending block of size ", str(
                    hex(applen)), " from ", str(hex(start)), " to ", str(hex(end)))
                end = end - applen

                update = bytearray([0x00]*16)
                fill_word(update, 0, ((20 << 16) |
...
#                        print(str(hex(start+x)), " to ", str(hex(start + x + maxChunkSize)))
                    ...
                    fill_word(dataMsg, 0, (((chunklen + 12) << 16)
...
#    print([hex(n) for n in params])
    ...
    if len(response) != response_len:
        verboseprint('No response for command 0x{:08X}'.format(
            word_from_bytes(params, 0) & 0xFFFF))
        n = len(response)
        if (n != 0):
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='UART Wired Update Host for Apollo3')
    ...
    #                     + str(AM_SECBOOT_WIRED_IMAGETYPE_CUSTPATCH) +
    #                     ': CustOTA, '
    #                     + str(AM_SECBOOT_WIRED_IMAGETYPE_NONSECURE) +
    #                     ': NonSecure, '
    #                     + str(AM_SECBOOT_WIRED_IMAGETYPE_INFO0) + ': Info0 '
    #                     + str(AM_SECBOOT_WIRED_IMAGETYPE_INFO0_NOOTA) +
    #                     ': Info0_NOOTA) '
    #                     + str(AM_SECBOOT_WIRED_IMAGETYPE_INVALID) +
    #                     ': Invalid) '
    #                     '- default[Invalid]')
