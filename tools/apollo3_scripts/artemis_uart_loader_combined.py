...
# May 30, 2019 - Arduino platform.txt supports only one bootload tool
# so we now call external bin_to_blob and ota_to_wire executables
# at the start of this script. This reduces compile time because we
# no longer need to call the executables during post processing every
# time the user compiles (but does not upload).
...
def main():
    ...
    print('Preparing binaries for transfer...', flush=True)

    loglevel = "0"
    if args.verbose:
        loglevel = "1"

    outputDirectory = os.path.dirname(args.binfile)

    #Run the bin to blob conversion 
    binToBlobTool = args.toolLocation + "/artemis_bin_to_blob.exe"
    nonsecureOTAfile = outputDirectory + "/main_nonsecure_ota"
    ...
    #Run the ota to wire conversion
    otaToWireTool = args.toolLocation + "/artemis_ota_to_wire.exe"
    inputFile = outputDirectory + "/main_nonsecure_ota.bin"
    wirefile = outputDirectory + "/main_nonsecure_wire"
    ...
    # We should now have a converted binary. The original script expected this file
    # through the args so repoint to new file.
    args.binfile = outputDirectory + "/main_nonsecure_wire.bin"
    ...
    while loadTries < 3: 
        ...
        with serial.Serial(args.port, args.baud, timeout=1) as ser:
            ...
            time.sleep(0.007) #3ms and 10ms work well. Not 50, and not 0.
...
if __name__ == '__main__':
    ...
    parser.add_argument('-toolLocation', dest='toolLocation',
                        help='Location of artemis tools')
